...
void InterruptManager::setInterruptDescriptor(uint32 index, uint32 address,
    ...
    IDT[index * 2 + 1] =
        (address & 0xffff0000) | (0x1 << 15) | (DPL << 13) | (0xe << 8);
}
...
extern "C" void c_time_interrupt_handler() {
    ...
    ++cur->realRuntime;
    ++cur->currentSliceTicks;
    cur->vruntime += (NICE_0_LOAD * VRUNTIME_SCALE) / cur->weight;

    // Also update the original ticks for backward compatibility
    if (cur->ticks) {
        --cur->ticks;
        ++cur->ticksPassedBy;
    }

    if (cur->currentSliceTicks >= cur->timeSlice) {
        programManager.scheduleCFS();
    }
}

void InterruptManager::enableInterrupt() { asm_enable_interrupt(); }

void InterruptManager::disableInterrupt() { asm_disable_interrupt(); }
